<template>
	<button class="flex items-center group max-w-full my-0 transition"
            :class="setClassNames()"
            :title="title"
            @click="click">
        <PawIcon v-if="icon"
    ...
    methods: {
        ...
        setIconClassNames() {
            ...
            if (this.disabled) {
                ...
                // Filled / Outlined
                if (this.outlined) {
                    className += 'text-gray-400 group-hover:text-gray-600 dark:group-hover:text-gray-200 group-active:text-gray-400 dark:group-active:text-gray-400 group-focus:text-action-400 dark:group-focus:text-action-400';
                    // Correct icon's position of only-icon button
                    if (this.icon && !this.hasSlot) {
                        className += '  -ml-px';
                    }
                } else {
                    className += 'text-action-100 dark:text-action-100 group-hover:text-white group-active:text-action-100 dark:group-active:text-action-100 group-focus:text-white dark:group-focus:text-white';
                }
            }
