    ...
    <div
        ...
        <template v-if="loading">
            <div v-if="label || info" class="flex flex-row">
                <label
                    v-if="label"
                    class="max-w-[80px] h-3.5 mr-1 bg-gray-400 loading-bubble"></label>
                <span
                    v-if="info"
                    class="max-w-[48px] h-3.5 ml-auto bg-gray-400 loading-bubble"></span>
            </div>
            ...
            <div
                ...
                <div
                    ...
                    :class="[
                        contextPosition === 'top' && multiline
                            ? 'self-start'
                            : '',
                        contextPosition === 'center' && multiline
                            ? 'self-center'
                            : '',
                        contextPosition === 'bottom' && multiline
                            ? 'self-end'
                            : '',
                    ]"></div>
                ...
                <template v-if="readonly">
                    <div
                        ...
                        :class="[
                            extraPosition === 'top' && multiline
                                ? 'self-start'
                                : '',
                            extraPosition === 'center' && multiline
                                ? 'self-center'
                                : '',
                            extraPosition === 'bottom' && multiline
                                ? 'self-end'
                                : '',
                        ]">
                        <span
                            class="bg-gray-400 loading-bubble max-w-[24px] w-full h-2"></span>
            ...
            <div v-if="notification || warning || error" class="ml-2 mt-1">
                <div
                    v-if="notification"
                    class="max-w-[120px] h-2 bg-gray-400 mb-2 loading-bubble"></div>
                <div
                    v-if="warning"
                    class="max-w-[120px] h-2 bg-warning-400 mb-2 loading-bubble"></div>
                <div
                    v-if="error"
                    class="max-w-[120px] h-2 bg-danger-500 dark:bg-danger-400 loading-bubble"></div>
            </div>
        </template>
        ...
        <template v-else>
                ...
                <div
                    ...
                    <span
                        ...
                        <textarea
                            ...
                            :readonly="readonly"
                            :tabindex="
                                (readonly && !isOpen) || item?.unselectable
                                    ? -1
                                    : null
                            "></textarea>
                    ...
                    <PawLink
                        ...
                        :icon="
                            select
                                ...
                                : ''
                        "
                        :tabindex="!showingResults ? -1 : null"></PawLink>
                </span>
            </div>
            ...
            <div
                ...
                class="absolute z-20 opacity-0 bottom-1 w-full translate-y-full h-1 border-x border-gray-200 dark:border-gray-600 bg-gray-200 dark:bg-gray-600 transition"></div>
            <div
                v-if="data"
                :class="[
                    showingResults
                ...
                <div
                    class="max-h-[inherit] overflow-y-auto border-gray-300 dark:border-gray-700 shadow-box dark:shadow-box-dark"
                    :class="addLabel ? 'rounded-b-none' : 'rounded-b-md'">
                    <div
                        ...
                        <PawCrazyInput
                            ...
                            @option-moved="
                                (currentItem, direction) =>
                                    !item.unselectable
                                        ? moveOption(currentItem, direction)
                                        : null
                            "
                            @option-typed="(v) => optionTyped(v)"
                            @dropdown-closed="
                                !item.unselectable ? closeDropdown() : null
                            "
                            :context-control="
                                !item.unselectable
                                    ? select
                            ...
                            "
                            :context-checked="
                                !item.unselectable ? isItemChecked(item) : null
                            "
                            :context-disabled="
                                !item.unselectable ? item.disabled : null
                            "
                            :context-position="
                                !item.unselectable
                                    ? item.contextPosition
                                    : contextPosition
                            "
                            :extra-label="
                                !loadingResults ? item.extraLabel : extraLabel
                            "
                            :extra-icon="
                                !loadingResults ? item.extraIcon : extraIcon
                            "
                            :extra-icon-outlined="item.extraIconOutlined"
                            :extra-position="item.extraPosition"
                            :multiline="
                                !loadingResults ? item.multiline : multiline
                            "
    ...
    export default {
        ...
        props: {
            ...
            contextPosition: {
                type: String,
                validator: function (value) {
                    return [undefined, "top", "center", "bottom"].includes(
                        value
                    );
                },
                default: "top",
        ...
        methods: {
                ...
                if (!this.readonly && this.data) {
                    event: switch (event.code) {
                        case "ArrowUp":
                            ...
                            if (
                                this.data.filter((obj) => obj.unselectable)
            ...
            moveOption(item, direction, forceOpen) {
                ...
                this.focusOption(
                    this.getOptionInput(this.data[newIndex]),
                    forceOpen
                );
            },
            ...
            removeChip(item) {
                // Remove from chips
                let index = this.chips.findIndex((obj) => obj.id === item.id);
            ...
            cancelLoading(forceCancel) {
                if (
                    (!this.showingResults && this.loadingResults) ||
                    forceCancel
                ) {
                    this.$emit("cancelled");
                    this.loadingResults = false;
